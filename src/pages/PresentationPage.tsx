...
const PresentationPage = () => {
  ...
  return (
    <div className="presentation-page bg-white text-gray-900 min-h-screen print:text-[9pt] max-w-[210mm] mx-auto px-[15mm] py-[8mm]">
      ...
      <section className="kp-no-break mb-3">
        <div className="relative rounded-xl text-white p-4 overflow-hidden" style={{ backgroundColor: '#0a1628' }}>
          ...
          <div className="relative z-10 flex items-center gap-4">
            <div className="flex-1">
              <img src={logoCrm82} alt="CRM82" className="h-5 mb-2 brightness-0 invert" />
      ...
      <section className="mb-3 kp-no-break">
        <h2 className="text-sm font-extrabold mb-2 border-b border-gray-200 pb-1">Почему выбирают CRM82?</h2>
        <div className="grid grid-cols-4 gap-1.5 mb-2">
          {[
            ...
            <div key={item.label} className="text-center p-1.5 rounded-md bg-gray-50 border border-gray-100">
              <item.icon className="w-3.5 h-3.5 text-blue-600 mx-auto mb-0.5" />
        ...
        <div className="flex gap-1.5 mb-2">
          {[
            ...
            <div key={cert.alt} className="flex-1 rounded-md border border-gray-100 bg-gray-50 p-1 aspect-[4/3] flex items-center justify-center">
              <img src={cert.src} alt={cert.alt} className="max-w-full max-h-full object-contain" />
            </div>
          ))}
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-md p-1.5 text-[10px] text-blue-800 text-center font-medium">
      ...
      <section className="kp-page-break mb-3 kp-no-break">
        <h2 className="text-sm font-extrabold mb-2 border-b border-gray-200 pb-1">Знакомые ситуации?</h2>
        <div className="grid grid-cols-2 gap-1.5">
          {[
            ...
            <div key={item.text} className="flex gap-1.5 items-start p-2 rounded-md bg-red-50 border border-red-100">
              <item.icon className="w-3 h-3 text-red-500 mt-0.5 flex-shrink-0" />
              <p className="text-[10px] text-gray-700 leading-snug">{item.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-3 kp-no-break">
        <h2 className="text-sm font-extrabold mb-2 border-b border-gray-200 pb-1">Что вы получите после внедрения?</h2>
        <div className="grid grid-cols-2 gap-1.5 mb-2">
          {[
            ...
            { icon: BarChart3, title: "Прозрачная аналитика", desc: "Дашборды по каждому этапу" },
            { icon: Bot, title: "Автоматизация рутины", desc: "Воронки и боты за менеджеров" },
          ].map((item) => (
            <div key={item.title} className="kp-no-break p-2 rounded-md bg-green-50 border border-green-100">
              <div className="flex items-center gap-1 mb-0.5">
                <item.icon className="w-3 h-3 text-green-600" />
                <p className="font-bold text-[10px]">{item.title}</p>
              </div>
              <p className="text-[9px] text-gray-600">{item.desc}</p>
            </div>
          ))}
        </div>
      ...
      {/* ===== СТРАНИЦА 3: Услуги + Процесс + Ниши ===== */}

      <section className="kp-page-break mb-3 kp-no-break">
        <h2 className="text-sm font-extrabold mb-2 border-b border-gray-200 pb-1">Все интеграции в одном окне</h2>
        <div className="grid grid-cols-2 gap-1.5">
          {[
            { icon: Settings, title: "Настройка логики", desc: "Воронки, карточки сделок." },
            { icon: MessageSquare, title: "Мессенджеры", desc: "WhatsApp, Telegram, Instagram." },
            { icon: Headphones, title: "IP-телефония", desc: "Sipuni, АТС с записью." },
            { icon: Globe, title: "Сайт и почта", desc: "Заявки с форм и email." },
            { icon: Zap, title: "Автоматизация", desc: "Рассылки, автозадачи, документы." },
          ].map((item) => (
            <div key={item.title} className="flex gap-1.5 items-start p-1.5 rounded-md bg-gray-50 border border-gray-100">
      ...
      <section className="mb-3 kp-no-break">
        ...
        <div className="grid grid-cols-4 gap-1.5 mt-1.5">
          {[
            { num: "01", title: "Аудит", desc: "Анализ процессов (бесплатно)." },
            { num: "02", title: "Настройка", desc: "Воронки и интеграции." },
            { num: "03", title: "Обучение", desc: "Тренинги команды." },
            { num: "04", title: "Поддержка", desc: "30 дней после запуска." },
          ].map((s) => (
            <div key={s.num} className="text-center p-1.5 rounded-md bg-gray-50 border border-gray-100">
              <span className="text-lg font-extrabold text-blue-600/25">{s.num}</span>
              <p className="font-bold text-[10px]">{s.title}</p>
              <p className="text-[9px] text-gray-600 leading-snug">{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-3 kp-no-break">
        <h2 className="text-sm font-extrabold mb-2 border-b border-gray-200 pb-1">Опыт в вашей сфере</h2>
        <div className="grid grid-cols-3 gap-1.5">
          {[
            { icon: Building2, title: "Недвижимость", desc: "ЮГ-1, АН Морозов" },
            ...
            <div key={item.title} className="flex gap-1.5 items-center p-1.5 rounded-md bg-gray-50 border border-gray-100">
      ...
      <section className="kp-page-break mb-3 kp-no-break">
        <h2 className="text-sm font-extrabold mb-2 border-b border-gray-200 pb-1">Кейсы</h2>
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <div key={c.title} className="p-2 rounded-md bg-gray-50 border border-gray-100 text-center">
              <div className="w-8 h-8 rounded bg-blue-100 flex items-center justify-center mx-auto mb-1">
                <span className="text-xs font-extrabold text-blue-600">{c.percent}%</span>
              </div>
              <p className="font-bold text-[10px]">{c.title}</p>
              <p className="text-[10px] text-blue-700 font-semibold">{c.result}</p>
      ...
      <section className="kp-no-break">
        <div className="rounded-xl bg-gradient-to-br from-gray-900 via-blue-950 to-gray-900 text-white p-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              <img src={logoCrm82} alt="CRM82" className="h-5 mb-1 brightness-0 invert" />
          ...
          <div className="flex gap-2 mb-2">
            <a href="https://t.me/crm82_support" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 bg-blue-600/30 border border-blue-500/30 rounded-md px-3 py-1.5 text-[10px] text-blue-200">
              <Send className="w-3 h-3" />
              Telegram
            </a>
            <a href="https://wa.me/79697773672" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 bg-green-600/30 border border-green-500/30 rounded-md px-3 py-1.5 text-[10px] text-green-200">
              <MessageCircle className="w-3 h-3" />
              WhatsApp
            </a>
          </div>
          <p className="text-[9px] text-blue-300/60">ИП Рудольф Евгений Евгеньевич</p>
        </div>
      </section>
    </div>
  );
